...
#ifndef ANDROID
	#define	NvFile				FILE
	#define	NvFOpen(p,q)		fopen(p,q)
	#define	NvFClose(p)			fclose(p)
	#define	NvFChdir(p)			fchdir(p)
	#define	NvFGetc(p)			fgetc(p)
	#define	NvFGets(p,q,r)		fgets(p,q,r)
	#define	NvFSize(p)			fsize(p)
	#define	NvFSeek(p,q,r)		fseek(p,q,r)
	#define	NvFTell(p)			ftell(p)
	#define	NvFRead(p,q,r,s)	fread(p,q,r,s)
	#define	NvFEOF(p)			feof(p)
