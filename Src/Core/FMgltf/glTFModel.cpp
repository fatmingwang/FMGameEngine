...
void cglTFModel::RenderNodesShadowPass(const cMatrix44& lightViewProj, GLuint shadowShaderProgram, int e_iLightType)
{
    for (auto l_Node : m_TopNodesVector)
    {
		l_Node->RenderNodesShadowPass(lightViewProj, shadowShaderProgram, e_iLightType);
    }
}
...
void cglTFModel::DebugRender()
{
    ...
    if (l_pCamera)
    {
        auto l_matWVP = l_pCamera->GetWorldViewProjection();
