...
void   cglTFNodeData::RenderNodesShadowPass(const cMatrix44& lightViewProj, GLuint shadowShaderProgram,int e_iLightType)
{
    ...
    if (m_pMesh)
    {
		...
		if (e_iLightType == (int)eLightType::eLT_DIRECTIONAL)
        {
   //         auto l_vDirection = lightViewProj.GetTranslation()- modelMatrix.GetTranslation();
			////10 should be bounding sphere radius or larger
   //         l_vDirection = l_vDirection.Normalize()*0.13;
   //         auto l_CopyMatrix = cMatrix44(lightViewProj);
   //         l_CopyMatrix.SetTranslation(l_vDirection);
   //         // For directional lights, we might want to apply an offset to avoid shadow acne
   //         //cMatrix44 biasMatrix = cMatrix44::TranslationMatrix(Vector3(0.0f, 0.0f, 0.01f)); // Small offset along light direction
   //         //modelMatrix = biasMatrix * modelMatrix;
   //         m_pMesh->RenderShadow(l_CopyMatrix, modelMatrix, shadowShaderProgram);
    ...
    for (Frame* child = GetFirstChild(); child; child = child->GetNextSibling())
    {
        ...
        if (childObj)
        {
            childObj->RenderNodesShadowPass(lightViewProj, shadowShaderProgram, e_iLightType);
        }
        else
        {
            ...
            if (l_pglTFNodeData)
            {
                l_pglTFNodeData->RenderNodesShadowPass(lightViewProj, shadowShaderProgram, e_iLightType);
            }
        }
    }
}
