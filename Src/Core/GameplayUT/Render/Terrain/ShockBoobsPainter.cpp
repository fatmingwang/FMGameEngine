...
namespace FATMING_CORE
{
	...
	void	cShockBoobsPainter::TerrainTransform(cFrameCamera*e_pFrameCamera)
	{
		...
		l_vScale.x = cGameApp::m_spOpenGLRender->m_vGameResolution.x/m_pTerrianVertex->GetGridWidth();
		l_vScale.y = cGameApp::m_spOpenGLRender->m_vGameResolution.y/m_pTerrianVertex->GetGridHeight();
		if( m_pPaintImage )
		{
			if( !e_pFrameCamera )
			{
	...
	void	cShockBoobsPainter::MouseUp(int e_iPosX,int e_iPosY)
	{
		...
		if( m_ePatinerMode != ePM_PAINT )
		{
			...
			if( l_fLength == 0 )
				return;
			float	l_fStregth = l_fLength/cGameApp::m_spOpenGLRender->m_vGameResolution.Length();
			l_fStregth *= 2;
	...
	bool	cShockBoobsPainter::ChangePaintingImage(const char*e_strFileName,bool e_bAutoResizeTerrianToGameResoution)
	{
		...
		if( e_bAutoResizeTerrianToGameResoution )
		{
			...
			if( l_bIsWidthBigger )
			{
				l_fScale = cGameApp::m_spOpenGLRender->m_vGameResolution.x/m_pOriginalPaintImage->GetWidth();
			}
			else
			{
				l_fScale = cGameApp::m_spOpenGLRender->m_vGameResolution.y/m_pOriginalPaintImage->GetHeight();
			}
