...
namespace FATMING_CORE
{
	cCameraZoomFunction::cCameraZoomFunction(const char*e_strTestBGImageName)
	{
		...
		{
			Vector2	l_vMax(cGameApp::m_spOpenGLRender->m_vGameResolution.x,cGameApp::m_spOpenGLRender->m_vGameResolution.y);
			m_ZoomableCamera.SetViewData(l_vMax/10,
				Vector4(0,0,l_vMax.x,l_vMax.y),
				Vector4(0,0,l_vMax.x,l_vMax.y));		
		}
	...
	void	cCameraZoomFunction::Update(float e_fElpaseTime)
	{
		...
		if(l_bDoPointsPosHit)
		{
			...
			m_vCenterPos = cOrthogonalCamera::ConvertMousePositionToWorldPosition(l_vCenterPos,cGameApp::m_spOpenGLRender->m_vViewPortSize.Size(),m_ZoomableCamera.GetViewRect());
		}
	...
	void		cCameraZoomFunction::Render()
	{
		...
		glEnable2D(cGameApp::m_spOpenGLRender->m_vViewPortSize.Width(),cGameApp::m_spOpenGLRender->m_vViewPortSize.Height());
	...
	{
#ifdef WIN32
		Vector2	l_vGameResolution = cGameApp::m_spOpenGLRender->m_vGameResolution;
		if( m_pBGImage )
		{
			cGameApp::m_spOpenGLRender->m_vGameResolution.x = (float)m_pBGImage->GetWidth();
			cGameApp::m_spOpenGLRender->m_vGameResolution.y = (float)m_pBGImage->GetHeight();
		}
		...
		cGameApp::m_spOpenGLRender->m_vGameResolution = l_vGameResolution;
		m_vGameMousePos = cOrthogonalCamera::ConvertMousePositionToWorldPosition(Vector2(e_iPosX,e_iPosY),cGameApp::m_spOpenGLRender->m_vViewPortSize.Size(),m_ZoomableCamera.GetViewRect());
#endif
	}
	...
	void		cCameraZoomFunction::KeyUp(unsigned char e_ucKey)
	{
		...
		{
			Vector2	l_vSize = cGameApp::m_spOpenGLRender->m_vViewPortSize.Size();
		...
		if( e_ucKey == 13 )
		{
			...
			float	l_fWidth = cGameApp::m_spOpenGLRender->m_vGameResolution.x;
			float	l_fHeight = cGameApp::m_spOpenGLRender->m_vGameResolution.y;
			if( m_pBGImage )
			{
				float	l_fWidth = (float)m_pBGImage->GetWidth();
				float	l_fHeight = (float)m_pBGImage->GetHeight();
